...
**自动部署（GitHub Actions）**：push 到 `main` 分支自动触发 `.github/workflows/deploy.yml`——打包源码 → SSH 上传到服务器 → 重建镜像并重启容器 → 等待健康检查 → 公网验证。仓库只需配置一个 Secret：`SSH_DEPLOY_KEY`（CI 专用私钥，公钥在服务器 `authorized_keys`；粘贴带围栏或多余空白也能被工作流自动清洗）。服务器地址与用户名硬编码在工作流中。构建失败时旧容器继续运行、旧源码自动还原；每次运行（无论成败）会把执行日志提交到 `ci-diag` 分支供排查。
